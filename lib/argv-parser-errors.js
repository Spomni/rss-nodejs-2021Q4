...
class FirstArgumentError extends InputError {
  constructor(name) {
    super('FirstArgumentError: first argument should be an option')
  }
}
...
class FlagOptionValueError extends InputError {
  constructor(name) {
    super(`FlagOptionValueError: the "${name}" option is a flag and should not have any value`)
  }
}

class UnexpectedValuesArrayError extends InputError {
  constructor(name) {
    super(`UnexpectedValuesArrayError: the option "${name}" should be assigned with one value only`)
  }
}

class MissedRequiredOptionError extends InputError {
  constructor(name) {
    super(`MissedRequiredOptionError: the option ${name} should not be missed`)
  }
}

class MissedOptionValueError extends InputError {
  constructor(name) {
    super(`MissedOptionValueError: the option "${name}" should has a value`)
  }
}
